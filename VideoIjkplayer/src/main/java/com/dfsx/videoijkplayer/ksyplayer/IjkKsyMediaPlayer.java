...
public class IjkKsyMediaPlayer extends AbstractMediaPlayer {
     ...
     */
    public IjkKsyMediaPlayer(Context context, boolean isUseHard, float bufferTimeMax) {
    ...
    private class KsyMediaPlayerListenerHolder implements IMediaPlayer.OnPreparedListener, IMediaPlayer.OnCompletionListener,
        ...
        public boolean onInfo(IMediaPlayer iMediaPlayer, int i, int i1) {
            if (i == IMediaPlayer.MEDIA_INFO_SOFTWARE_DECODE) {
                isHardwareDecode = false;
            } else if (i == IMediaPlayer.MEDIA_INFO_HARDWARE_DECODE) {
                isHardwareDecode = true;
            } else if (i == IMediaPlayer.MEDIA_INFO_VIDEO_ROTATION_CHANGED) {
                if (mInternalMediaPlayer != null && !isHardwareDecode) {
                    mInternalMediaPlayer.setRotateDegree(180);
                }
            }
            IjkKsyMediaPlayer self = mWeakMediaPlayer.get();
