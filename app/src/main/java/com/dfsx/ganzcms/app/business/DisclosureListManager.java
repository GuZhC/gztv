...
/**
 * Created by heyang on 2016/8/31
 */
public class DisclosureListManager extends DataFileCacheManager<ArrayList<DisclosureItem>> {
    ...
    public DisclosureListManager(Context context, String accountId, int columnType) {
        super(context, accountId, DISCLOSURELISTMANAGER + "_" + columnType);
        mSession = App.getInstance().getmSession();
        this.mColumnType = columnType;
    }

    public DisclosureListManager(Context context, String accountId, int columnType, boolean isLooper) {
        super(context, accountId, DISCLOSURELISTMANAGER + "_" + columnType + (isLooper ? "_LooperImg" : ""));
        mSession = App.getInstance().getmSession();
        this.mColumnType = columnType;
    }
    ...
    public void start(int  columnId,boolean bAddTail, int offset) {
        String param = "/public/columns/{column-id}/threads?";
        getDocObjectParam(bAddTail, param, offset);
    }
