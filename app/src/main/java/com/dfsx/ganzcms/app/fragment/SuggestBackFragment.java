...
/**
 * Created by heyang on 2017/10/24.
 */
public class SuggestBackFragment extends Fragment implements View.OnClickListener, FragmentBackHandler, PictureManager.OnDataComplateInitLister, DefaultFragmentActivity.ViewClickLister, EditChangedLister.EditeTextStatuimpl {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        rootView = inflater.inflate(R.layout.frag_suggestion_custion, null);
...
//            }
    ...
    public void createTopical(ArrayList<MediaModel> list) throws ApiException {
        isCopmlate = false;
        String content = mContenEdt.getText().toString().trim();
        _accountApi.publisSuggest(content, list, new ProgessCallbacface() {
            @Override
            ...
            public void onNext(Long aLong) {
                ...
                if (aLong != -2) {
//                    RxBus.getInstance().post(new Intent(IntentUtil.ACTION_UPDTA_ATTEION_OK));
                    Toast.makeText(getActivity(), "意见反馈提交成功", Toast.LENGTH_LONG).show();
                    PictureManager.getInstance().clearCache();
                    getActivity().finish();
                } else{
...
//        exeUploadTast();
        if (list == null) {
            list = PictureManager.getInstance().getmDllist();
        }
        try {
            createTopical(list);
        } catch (ApiException e) {
