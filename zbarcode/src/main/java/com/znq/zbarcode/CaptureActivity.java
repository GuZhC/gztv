...
public class CaptureActivity extends Activity implements SurfaceHolder.Callback {
    ...
    {
        camera = mCameraManager.getCamera(); //我们先前在CameraManager类中添加的静态方法
    ...
    private void initView() {
        flashBtn = (ImageView) findViewById(R.id.open_flash_btn);
    ...
    public void checkResult(final String result) {
        ...
        int duration = 1000;
        if (beepManager != null)
            duration = beepManager.getTimeDuration();
    ...
    //设为protected，以便子类继承重写处理result
    protected void activityResult(String result) {
        if (!isFinishing()) {
...
//            Toast.makeText(MainTabActivity.this, result + "", Toast.LENGTH_SHORT).show();

            Intent in = new Intent(CaptureActivity.this, ResultsActivity.class);
